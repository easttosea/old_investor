...
#        self.init_fund_info()
    ...
    def init_fund_code(self):
        ...
        cursor = conn.cursor()
        cursor.execute('SELECT a_code, b_code FROM structured_fund_info WHERE list_date IS NOT NULL')
        table = cursor.fetchall()
        for row in table:
            if row[0] not in self.fund_a_code:
                self.fund_a_code.append(row[0])
            if row[1] not in self.fund_b_code:
                self.fund_b_code.append(row[1])
        cursor.close()
        conn.close()
