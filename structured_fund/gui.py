...
class MyWindow(QtWidgets.QMainWindow, Ui_Form):
    ...
    def fill_table_list(self):
        self.frame_realtime = self.frame_realtime.sort_values(by='modified_rate_of_return', ascending=False)
        ...
        for fund in data_list:
            cell_list = [QtWidgets.QTableWidgetItem(data) for data in fund]
            if fund[7] in self.RATE_RULE_SHOW:
                cell_list[7].setForeground(self.RATE_RULE_SHOW[fund[7]])
            if self.frame_realtime.a_increase_value[row] > 0:
                cell_list[2].setForeground(self.COLOR_RED)
                cell_list[3].setForeground(self.COLOR_RED)
            elif self.frame_realtime.a_increase_value[row] < 0:
                cell_list[2].setForeground(self.COLOR_GREEN)
                cell_list[3].setForeground(self.COLOR_GREEN)
...
#        self.selected_b_code = self.frame_realtime.at[self.selected_mother_code, 'b_code']
    ...
    def fill_table_handicap(self):
        ...
        for data_serial in range(0, len(data_a_list), 2):
            ...
            self.tableWidget_handicap.setItem(row, 0, cell_price)
            self.tableWidget_handicap.setItem(row, 1, cell_volume)
            row += 1
